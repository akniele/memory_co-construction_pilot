...
def load_data(data_file):
    ...
    df = pd.read_csv(myfile, sep="\t", quotechar='"') # quoting=csv.QUOTE_NONE
...
def load_annotations(user_folder,index):
    ...
    if os.path.isfile(annotation_filepath):
        with open(annotation_filepath, "r", encoding='utf-8') as f:
            ...
            existing_relations = annotations_data.get("Relations", None)
            print(f"existing_annotations: {existing_annotations}")
            print(f"existing_relations: {existing_relations}")

    return existing_annotations, existing_relations
...
def annotate(index, email):
    ...
    annotation_title, guidelines_file, data_file,\
        text1_column, text2_column, summary_column, text_id_column,\
    ...
    text1 = df.iloc[index][text1_column]
    text2 = df.iloc[index][text2_column]
    summary = df.iloc[index][summary_column]

    # Calculate the index for the next/back text
    next_index = index + 1
    prev_index = index -1

    existing_annotations, existing_relations = load_annotations(user_folder,index)
    ...
    if request.method == "POST":
        ...
        if action == "next":
            ...
            relation_spans = json.loads(relations) if relations else []

            if len(annotations)!=0:
                # Save the annotations
                save_annotation(user_folder, index, row, annotations, relation_spans)
            ...
            # Load annotations for the next index
            next_annotations, next_relations = load_annotations(user_folder, next_index)
            next_annotations_json = json.dumps(next_annotations)  # Ensure proper JSON formatting
            next_relations_json = json.dumps(next_relations)  # Ensure proper JSON formatting
            return redirect(url_for("annotation.annotate", index=next_index, email=email, annotations=next_annotations_json, relations=next_relations_json))
        ...
        if action == "back" and prev_index >= 0:
            ...
            # Load annotations for the previous index
            prev_annotations, prev_relations = load_annotations(user_folder, prev_index)
            prev_annotations_json = json.dumps(prev_annotations)  # Ensure proper JSON formatting
            prev_relations_json = json.dumps(prev_relations)  # Ensure proper JSON formatting
            return redirect(url_for("annotation.annotate", index=prev_index, email=email, annotations=prev_annotations_json, relations=prev_relations_json))
    ...
    if existing_relations is None:
        ...
        existing_relations_list = [value for key, value in existing_relations.items()]
    ...
    return render_template("annotation.html",
                           ...
                           existing_relations=existing_relations_list
                           )
